...
///
/// The `BigInt` type is fully generic on the size of its "word" -- the
/// `BigInt` alias uses the system's word-sized `UInt` as its word type, but
/// any word size should work properly.
public struct BigInt<Word: FixedWidthInteger> :
    ...
    where Word.Magnitude == Word
{
    ...
    public init?(_ source: String, radix: Int = 10) {
        ...
        let vs = source.utf16.map(valueForCodeUnit)
    ...
    public static func >>= <RHS : BinaryInteger>(lhs: inout BigInt, rhs: RHS) {
        ...
        // higher word and the high `BigInt.Word.bitWidth - offset` bits of
        // the lower word.
